...
function LX = fbtxsparc(t,L)
 ...
 LX.t       = 4;            % Time in pulse (s)
 CS1_current = 3012;        % Amps
 
 %% Equilibrium scalars
 LX.rBt     = 12.2*P.r0;    % R B_T (Tesla - m)
 LX.Ip      = 4.0e6 ;       % Plasma current (A)
 LX.qA      = 1.0;          % Central q
 LX.bp      = 0.063;        % Beta poloidal (%)
 ...
 a0 = 0.55;                 % Midplane minor radius (m)
 ...
 X_flux = -.85;               % Flux at the X-points

 sp_flux = -.85;            % Flux at strikepoints
 ...
 LX.gpdw = 8*ones(na,1);      % Simple current dipoles
