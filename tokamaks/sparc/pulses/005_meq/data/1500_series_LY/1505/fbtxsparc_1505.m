...
function LX = fbtxsparc(t,L)
 ...
 LX.t       = 5;            % Time in pulse (s)
 CS1_current = -6399;       % Amps
 
 %% Equilibrium scalars
 LX.rBt     = 12.2*P.r0;    % R B_T (Tesla - m)
 LX.Ip      = 5.0e6 ;       % Plasma current (A)
 LX.qA      = 1.0;          % Central q
 LX.bp      = 0.058;        % Beta poloidal (%)
 ...
 a0 = 0.555;                 % Midplane minor radius (m)
 ...
 X_flux = -.6;               % Flux at the X-points

 sp_flux = -.6;             % Flux at strikepoints
 ...
 LX.gpdw = 6*ones(na,1);      % Simple current dipoles
