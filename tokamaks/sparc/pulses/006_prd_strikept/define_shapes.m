...
% add strike points to target control points
rstrike = [eqs(:).rstrike]';
zstrike = [eqs(:).zstrike]';

% hold strike points for times when its limited
i = find(~[eqs(:).islimited]);
rstrike = interp1hold(i, rstrike(i,:), 1:length(eqs));
zstrike = interp1hold(i, zstrike(i,:), 1:length(eqs));
